...
DATA_SOURCE_NAME = 'LEAD-API-DB-FULL-CUSTOMER'
...
def loadTask(job):
    try:
        ...
        if job["RuntimeMode"].upper() == 'FULL_LOAD':
            ...
            date_created = spark.sql(f"select upper(cast(date_format(cast(max({job['CheckPointCol']}) as timestamp), 'dd-MMM-yyyy HH:mm:ss') as varchar(20))) as date_created from {job['TargetDB']}.{job['TargetTable']}").first()["date_created"]
            
            print(date_created)
            print('Executing: ', eval(job["SqlQueryTxn"]))
            ...
            df = spark.read.format("jdbc") \
                ...
                .option("url", f"jdbc:oracle:thin:{user}/{password}@{host}:{port}/{sid}") \
                .option("dbtable", eval(job["SqlQueryTxn"])) \
                .option("user", user) \
...
def main():
    try:
        ...
        # for job in jobs[10:11]:
        # for job in jobs[11:12]:
        print('2')
        for job in jobs:
